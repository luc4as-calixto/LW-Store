...
try {
    ...
    $sqlCount = "SELECT COUNT(*) FROM sellers";

    if (!empty($termo)) {
        $sqlCount .= " WHERE name LIKE :termo OR email LIKE :termo";
    }
    ...
    $totalVendedores = $stmtCount->fetchColumn();
    $totalPaginas = ceil($totalVendedores / $limite);
